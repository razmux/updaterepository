...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	{ //Check rivalry settings.
		...
		if( flag&(BCT_PARTY|BCT_ENEMY) )
		{
			...
			if( s_party && s_party == status_get_party_id(t_bl) && !(mapdata->flag[MF_PVP] && mapdata->flag[MF_PVP_NOPARTY]) && !(map_flag_gvg(m) && mapdata->flag[MF_GVG_NOPARTY]) && !mapdata->flag[MF_BATTLEGROUND] )
				state |= BCT_PARTY;
